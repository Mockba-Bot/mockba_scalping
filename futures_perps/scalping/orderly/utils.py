...
def get_klines(symbol: str, interval: str, limit: int = 100) -> List[List]:
    """
    Fetch klines from Orderly: returns list of [timestamp, open, high, low, close, volume]
    """
    public_rate_limiter()
    url = f"{BASE_URL}/v1/public/kline"
    params = {"symbol": symbol, "type": interval, "limit": min(limit, 1000)}
    try:
